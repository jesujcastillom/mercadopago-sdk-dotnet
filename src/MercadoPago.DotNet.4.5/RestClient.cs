...
namespace MercadoPago.DotNet
{
    internal static class RestClient
    {
        ...
        internal static async Task<JObject> Exec(HttpMethod method, string uri, JObject data, IEnumerable<KeyValuePair<string, string>> parameters, string contentType)
        {
        ...
        internal static async Task<Response<T>> Exec<T>(HttpMethod method, string uri, JObject data, IEnumerable<KeyValuePair<string, string>> parameters, string contentType)
        {
            using (var client = new HttpClient())
            {
                client.BaseAddress = new Uri(API_BASE_URL + UriPrefix + uri);
                ...
                var payload = new JsonSerializer().Deserialize<T>(new JsonTextReader(new StringReader(responseJson)));

                var result = new Response<T>(response.StatusCode, payload);
